...
void main() {
  void mockVersionSelector(gg.VersionSelector versionSelector) =>
      when(
  ...
  group('gg_multi review integration', () {
    test('adds a & b and reviews them end-to-end', skip: true, () async {
      final logs = <String>[];
      ...
      try {
        ...
        final initRunner =
            CommandRunner<void>('gg_multi', 'gg_multi integration init')
              ..addCommand(
                InitWorkspaceCommand(ggLog: ggLog, rootPath: tempRoot.path),
              );
        ...
        for (final projectName in <String>['a', 'b']) {
          ...
          final oceanRepoDir = Directory(path.join(oceanDir.path, projectName));
        ...
        for (final projectName in <String>['a', 'b']) {
          ...
          expect(
            ...
            isTrue,
            reason:
                'pubspec.yaml of package $projectName in '
        ...
        print('------- Running gg_multi create ticket $ticketName -------');
        ...
        print('------- Running gg_multi add a b -------');

        final addRunner =
            CommandRunner<void>('gg_multi', 'gg_multi integration add')
              ..addCommand(
                AddCommand(
                  ggLog: ggLog,
        ...
        print('------- Running gg_multi do commit -------');
        ...
        print('------- Running gg_multi do push -------');

        final doPushRunner = CommandRunner<void>(
          'gg_multi',
          'gg_multi integration do push',
        )..addCommand(DoPushCommand(ggLog: ggLog));
        ...
        print('------- Running gg_multi can review -------');
        ...
        print('------- Running gg_multi do review -------');

        final reviewRunner = CommandRunner<void>(
          'gg_multi',
          'gg_multi integration do review',
        )..addCommand(DoReviewCommand(ggLog: ggLog));
        ...
        print('------- Running gg_multi can commit -------');
        ...
        print('------- Running gg_multi do publish -------');
        ...
        final publishRunner =
            CommandRunner<void>('gg_multi', 'gg_multi integration do publish')
              ..addCommand(
                DoPublishCommand(
                  ggLog: ggLog,
...
Future<void> _ensureBinaryAvailable(String binary) async {
  final result = await Process.run(binary, <String>[
    '--version',
  ], runInShell: true);
  ...
  expect(
    ...
    0,
    reason:
        '$binary must be available on PATH for this integration test. '
...
Future<void> _initializeGitRepoWithRemote({
  ...
  await _runGit(<String>[
    'commit',
    '-m',
    'Initial commit for $projectName',
  ], workingDirectory: repoDir.path);
