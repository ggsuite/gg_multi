...
import 'package:gg_multi/src/backend/gitignore_lock_files.dart';
...
void main() {
  ...
  const dartLockBlock = '# Avoid committing pubspec.lock for library packages; '
      'see\n'
  ...
  bool run() => ensureLockFilesNotIgnored(
        repoDir: repoDir,
        ggLog: messages.add,
      );
  ...
  group('ensureLockFilesNotIgnored', () {
    test('removes the entry together with the comment block above it', () {
      ...
      expect(
        messages,
        contains(
          'Removed the lock file entries from .gitignore of my_repo.',
        ),
      );
    });
