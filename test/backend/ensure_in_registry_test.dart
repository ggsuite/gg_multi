...
void main() {
  ...
  group('EnsureInRegistry', () {
    group('ensure(directory, ggLog)', () {
      test('does nothing when the package is already in the registry',
          () async {
        mockInRegistry([true]);
      ...
      test(
          'asks for a manual first publish of a Dart package '
          'and continues once it is in the registry', () async {
        mockInRegistry([false, true]);

        await ensureInRegistry(answers: ['']).ensure(
          directory: d,
          ggLog: ggLog,
        );
      ...
      test('shows the shell commands in blue', () async {
        ...
        final blueMessages =
            coloredMessages.where((m) => m.startsWith('\x1B[34m'));
      ...
      test('throws when stdin is not a terminal', () async {
        ...
        await expectLater(
          ensureInRegistry(hasTerminal: false).ensure(
            directory: d,
            ggLog: ggLog,
          ),
          throwsA(
      ...
      test(
        ...
        expect(
          log,
          contains('pnpm publish --no-git-checks --access public'),
        );
      });
      ...
      test('treats a null answer like an empty one', () async {
        ...
        await ensureInRegistry(answers: [null]).ensure(
          directory: d,
          ggLog: ggLog,
        );
