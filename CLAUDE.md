...
| `trash.dart`                 | `<root>/.trash/<ticket>` — where removed repos and workspace files go instead of being deleted. `moveFromMaster` adds the master side: a repo `do update master` drops lands in `<root>/.trash/.master/<org>/<repo>`                                                                                                                                                                                                                                                                                                                                                        |
...
### `do update master` Command

`UpdateMasterCommand` (in `lib/src/commands/do/update/master.dart`, grouped under `UpdateCommand` in `lib/src/commands/do/update.dart`) brings the master workspace back in sync with the git platforms. It reads every organization from `.organizations` — which lives **inside** `.master`, the path `do add` hands to `addRepositoryHelper` — asks each one for its current repository list (`GitHubPlatform.fetchOrgRepos` / `AzureDevOpsPlatform.fetchOrgRepos`, dispatched on `UrlParser().parse(org.url).platformType`, four organizations in parallel via `runWithLimit`), clones what master lacks and trashes what the organization no longer offers.
